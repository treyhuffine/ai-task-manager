...
- Robust and lasting. The browser is the persistent process and Ri is a thin client, so there is no daemon to leak or babysit.
...
A browser is always its own process. The persistent, stateful thing is the browser itself (its cookies, its tabs), and Ri is a client that connects to it. This is how Playwright's connectOverCDP works, how chrome-devtools-mcp autoConnect works, and how OpenClaw drives a real Brave today.

So Ri does not own a browser daemon. The rule is connect-or-launch:
...
Because Ri does not spawn or own the browser in the attended case, a dev hot-reload cannot orphan it and there is no zombie-process problem to manage. The only lifecycle logic is connect-or-launch, plus the single rule that a profile's data directory cannot be open twice at once (so a scheduled run reuses an already-open browser rather than fighting it for the lock).

Ri already ships this exact pattern for the Parakeet voice service (spawn on demand, reuse if warm, tear down on stop), so it is a known shape in the codebase, not a new concept.

## 4. One agent browser, logged in like any site

- Pick your flavor. `ri browser doctor` and a Settings dropdown list the installed Chromium-family browsers (Chrome, Brave, Edge, Chromium, plus the Playwright cache). The user picks one, it persists as `browserChromiumPath`. Brave and the rest drive by executable path. The default prefers a real installed browser over downloading Playwright's Chromium, and the download, if ever needed, is consented and lazy, never at install time.
- Log in like any website. There is no per-site login catalog. The one primitive is "open the agent browser," headed, so the user can sign into whatever they want, whenever, including 2FA, with their password manager, exactly as in a normal browser. No credentials are ever typed into Ri.
...
Reuses Ri's existing two-tier split with almost no new infrastructure.
...
The security model is the one you would use for a human assistant. You decide which accounts it can touch by deciding what to log the agent browser into. The agent then acts freely within that scope. Ri does not cage the agent with origin allowlists, per-action approval prompts, or cross-origin taint. Those are the human-organization scaffolding Ri exists to remove, and the profile's login scope already bounds the blast radius.

What Ri keeps, because it adds safety without restricting the agent or creating rot:
...
The two verbs plus auto-snapshot are the right, sufficient core for reading and acting, and the model drives them fluently. But being a real asset that replaces browser work needs three more things, and they are mostly about composing with Ri's existing spine rather than adding browser surface.

1. File transfer, the highest-value addition. A person's real browser work is full of files. Download an invoice, a receipt, a report, a boarding pass. Upload a resume, a document, an image. Ri is a notes app with a first-class attachment system, so this is exactly where browser work becomes durable Ri artifacts. It fits the two verbs without a third:
   - Upload is an act kind. `browser_act(kind: "upload", ref, attachment_id)` sets a file input from a Ri attachment.
   - Download is a session behavior. When an act triggers a download, the browser layer captures the file straight into Ri's attachment store and returns the attachment id, so "download my invoice and file it" ends with a real attachment on a note. This is a small addition and it is what turns browsing into productive output.
...
What neither nailed, and where Ri wins: a genuinely simple log-in-once experience with no operator setup, and a security model that is one human decision (what to log in) rather than a machine of restrictions.
...
- Upload act kind wired to Ri attachments, and download capture into the attachment store.
