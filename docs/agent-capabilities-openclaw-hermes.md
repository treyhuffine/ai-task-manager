...
> Source study of two vendored runtimes, OpenClaw (`examples/openclaw`) and Hermes (`examples/hermes-agent`), and a proposal for what Ri should adopt. Includes a full ranked and tagged catalog of everything both runtimes give their agent, including the items we recommend skipping.
>
...
3. Proposal A: Browser as a first-class Ri capability
4. Proposal B: Capability roadmap for Ri
...
Ri needs to give its agents real-world reach (read a paywalled article, push a reminder to the user, turn a PDF into a note) without turning every new capability into a bespoke script. This doc does two things:

- Proposes a concrete design for baking **browser control** into Ri as a first-class, batteries-included capability, portable across machines and backends.
...
**Implication for Ri.** MCP is the right layer for handing capability to an outside agent (Claude Code, Codex). For capability we own inside Ri, the correct pattern mirrors the two runtimes: a native capability with a small stable tool surface, swappable backends behind an adapter registry, and configuration that lives in the app. MCP stays available as an optional export, off by default, for the case where someone wants to drive Ri from an external agent.
...
- `cdp`: Ri-native, Playwright `connectOverCDP(cdpUrl)` against any Chromium launched with `--remote-debugging-port`. Zero dependency on another runtime.
...
- `browserbase` (or Steel): a cloud browser, for headless Ri deployments (triggers, servers, no local display).
...
Browser is a built-in capability, on by default with auto-detection. Configuration surfaces in a Settings panel and persists to Ri config, the same way other settings land in a file. The user should never wire up an MCP server or edit JSON by hand. The only human action is a **Log in to a site** button.

```yaml
# persisted to Ri config. The Settings panel is the friendly front end
browser:
  backend: auto            # auto | cdp | openclaw | hermes | browserbase
  profile: ri              # dedicated user-data-dir, never the personal profile
...
Both runtimes agree: the human logs in once into a dedicated profile, and the agent never automates login (automated logins trip anti-bot defenses and can lock the account). Ri wraps this as `ri browser login <url>`, which opens the dedicated profile headful so the user signs in once. The session persists in that profile's user-data-dir.
...
- MVP, works on the current Mac today: the `cdp` adapter plus the `browser` config block plus `ri browser login` and `ri browser doctor` plus the rewritten `medium-review` skill. It attaches to the live OpenClaw Brave on port 18800 or launches its own Chromium. No other runtime required.
- Then: the `openclaw` and `hermes` adapters (auto-detect) and a `browserbase` adapter so scheduled and headless Ri runs can browse too.
...
- **BUILD**: implement now. Convergent across both runtimes, strong fit, and a genuine gap in Ri.
- **STRONG**: high value, but more effort or partial overlap with what Ri has.
- **LATER**: situational or a future capture source.
- **HAVE**: Ri already has an equivalent. Do not rebuild. Convergence validates the existing design.
...
8. **TTS spoken briefings.** Read the day's deck aloud. Ri already has STT for the other direction.
9. **Task-app import.** One-time or ongoing pull from Things, Apple Reminders, Notion, or Trello into Ri.

### SKIP for Ri

Desktop and computer-use control, mobile-UI control, smart home (Hue, Sonos, Home Assistant), image, video, and music generation, phone calls, and the twenty-plus chat-platform host adapters. Ri is not a chatbot host. It wants to notify, not to be Telegram.

### What Ri already has (HAVE)
...
| skills authoring at runtime (skill_workshop, skill_manage) | OC+HM | STRONG | Discover, load, and author reusable skills | Ri has skills, not runtime authoring |
...
| Sandboxes (crabbox, openshell, mxc) | OC | SKIP | Cloud or OS-level sandboxed execution | Ri uses workspaces |
...
| Capability | Source | Verdict | What the agent can do | Ri note |
|---|---|---|---|---|
| progress_card, show_widget | OC | LATER | Maintain a progress card and interactive widgets | A pattern Ri could borrow for the deck |
...
3. Configuration in the app Settings, persisted to a Ri config file, with sensible auto-detecting defaults so it is batteries-included.
4. Mutations and outward actions through the existing approval gate.
5. MCP as an optional outward export, off by default, for driving Ri from an external agent.
