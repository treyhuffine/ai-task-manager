...
`lifecycle_revision`. A caller may pass an expected revision for optimistic
concurrency; a stale one returns a stable `conflict`. Every command is recorded
in the append-only `task_lifecycle_commands` ledger with provenance.

`lifecycle_state_since` records when a task entered its current status (for
...
- Schema migration `drizzle/0016` is additive only (new columns + the ledger
  table). It was hand-edited off drizzle's table-rebuild because `tasks` is the
  external-content source of the `tasks_fts` FTS5 index, and a rebuild would
  reassign rowids and desync FTS. Existing `status` bytes are preserved verbatim.
