...
- Schema migration `drizzle/0016` is additive only (new columns + the two new
  tables) and is generated by drizzle, not hand-edited. The trick is that the
  schema deliberately does NOT change the `tasks.status` SQL default: it stays
  at the legacy `active` (via `default(sql`'active'`)`), and the "new tasks are
  todo" rule lives in `createTask` instead. SQLite cannot `ALTER` a column
  default, so had the schema asked for a `todo` default, drizzle would have had
  to rebuild the whole `tasks` table (its only option for a default change),
  which reassigns rowids and desyncs the `tasks_fts` FTS5 index that is keyed by
  rowid. Keeping the SQL default untouched lets drizzle emit pure
  `ALTER TABLE ADD COLUMN` / `CREATE TABLE` statements. The status enum values
  change is type-level only in SQLite (no CHECK), so it produces no SQL at all.
  Existing `status` bytes are preserved verbatim.
