...
Ri can receive Index 01 voice captures at:

```text
POST https://<your-ri-host>/api/webhooks/pebble
```

This is a dedicated capture adapter. It is separate from agent trigger webhooks because the Pebble app can set static headers, but it cannot calculate Ri's per-request HMAC signature.
...
Add it to the environment used to start Ri:

```dotenv
PEBBLE_WEBHOOK_SECRET=<generated-secret>
```

For local development, put it in `.env.local`. Restart Ri after changing the environment.
...
| Setting | Value |
|---|---|
| URL | `https://<your-ri-host>/api/webhooks/pebble` |
...
Choose a payload mode based on what you want Ri to retain:

| Pebble mode | Ri behavior |
|---|---|
...
| Recording only | Stores the M4A, creates a retryable placeholder immediately, then tries Ri's configured speech-to-text provider in the background. |

Use **Send test event** after saving. Ri acknowledges a valid test event without adding it to the Stream.
...
```bash
curl https://<your-ri-host>/api/webhooks/pebble \
...
- The raw multipart body is not stored because it can contain binary audio. Ri stores a JSON audit envelope with the timestamp, gesture, original transcription, transcription source, and audio metadata. Auth headers are never included.
- Audio is limited to 50 MiB. Ri enforces the request limit on bytes received even when `Content-Length` is absent or incorrect. Transcription text is limited to 200,000 characters.
...
Ri writes compact JSON metadata to the terminal or service log for each delivery. A normal capture produces `request received`, `payload validated`, and `capture created` entries. Test events, rejected requests, duplicates, and background transcription have their own entries.
