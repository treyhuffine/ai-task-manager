...
- Migration backups land in `getAppRoot()/backups` (respects `RI_ROOT`), not a hardcoded `~/ri/.work/backups`.
...
async function migrate() {
  ...
  const backupsDir = path.join(os.homedir(), 'ri', 'backups');
...
```bash
# 1. Stop the app (Ctrl-C any running `pnpm dev` / `ri start`)

# 2. Backup before schema migration
mkdir -p ~/ri/.work/backups
sqlite3 ~/ri/data.db \
  ".backup ~/ri/.work/backups/data.db.pre-executions-schema-$(date +%Y%m%d-%H%M%S)"
...
sqlite3 ~/ri/data.db \
  ".backup ~/ri/.work/backups/data.db.pre-executions-script-$(date +%Y%m%d-%H%M%S)"
...
sqlite3 ~/ri/data.db \
  ".backup ~/ri/.work/backups/data.db.pre-executions-drop-$(date +%Y%m%d-%H%M%S)"
...
Backups land in `~/ri/.work/backups/` (machine-local scratch — never synced, safe to delete). Use `sqlite3 .backup` (not raw `cp`) because better-sqlite3 runs in WAL mode and naive cp can miss un-checkpointed writes in the `-wal` sidecar.

Recovery for any step: `cp ~/ri/.work/backups/data.db.<tag> ~/ri/data.db && rm ~/ri/data.db-{wal,shm}`.
...
2. **A CLI verb for executions** (`ri execution list/show`). Keep CLI on chats and runs.
