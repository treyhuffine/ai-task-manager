...
- [ ] Prod cutover. Trey runs it from a terminal, because this session runs as an execution under the prod server and stopping the server would kill it mid-swap. Stop the app (Ctrl-C the `pnpm cli:dev start` terminal), then `pnpm tsx scripts/db-rebuild.ts --in-place ~/ri/data.db` (about 5 minutes; it refuses while anything holds the file open, and swaps only if every check passes), then `pnpm cli:dev start`.
