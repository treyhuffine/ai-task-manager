...
- Inert-default exception: a NOT NULL policy-ish column changed via a rowid-safe column swap (the FTS-safe way — see `tasks.status` and `drizzle/0016`) is structurally FORCED to keep a default, because `ADD COLUMN ... NOT NULL` needs one and SQLite cannot `DROP` it afterward without the full-table rebuild the swap exists to avoid. In that one case, keep the default equal to the query-layer policy value and treat it as inert (the query layer is still the single source of truth). This is the "treat any pre-existing one as inert" clause, not a license to add fresh policy defaults.
