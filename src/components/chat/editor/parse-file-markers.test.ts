...
describe('parseFileMarkers', () => {
  ...
  it('splits text around a single marker', () => {
    const att = ATT('019-abc.png', { mimeType: 'image/png' });
  ...
  it('matches file_names containing dots and dashes (uuidv7.ext form)', () => {
    const att = ATT('0193abcd-ef01-2345-6789-abcdef012345.tsx', { mimeType: 'text/plain' });
    const segs = parseFileMarkers(`see [[file:${att.fileName}]]`, [att]);
    expect(segs[1]?.kind).toBe('chip');
  });

  it('preserves text after the last marker', () => {
    const att = ATT('a.png', { mimeType: 'image/png' });
