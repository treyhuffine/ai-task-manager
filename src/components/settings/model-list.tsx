...
import { Check, ChevronRight, Loader2, MessageSquarePlus, Settings2 } from 'lucide-react';
...
import { useSaveHarnessModels } from '@/hooks/use-agent-harnesses';
        ...
        {hidden.length > 0 && (
          <>
            ...
            >
              ...
              />
              {showHidden ? 'Show less' : `${hidden.length} more`}
            </button>
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="text-[12px] font-medium text-foreground">{model.label}</div>
        {model.hint && (
          <div className="mt-0.5 text-[10.5px] leading-snug text-muted-foreground/80">{model.hint}</div>
        )}
      </div>
    </button>
  );
