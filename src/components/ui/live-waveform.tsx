...
export type LiveWaveformProps = HTMLAttributes<HTMLDivElement> & {
  active?: boolean
  processing?: boolean
  /** External stream — when provided, LiveWaveform uses this for visualization
   *  instead of calling getUserMedia. Caller is responsible for stopping tracks. */
...
export const LiveWaveform = ({
  ...
  // When `externalStream` is provided the caller owns the mic lifecycle —
  // LiveWaveform only hooks up an AnalyserNode for visualization.
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        ...
        // stop the stream immediately so the browser mic indicator goes away.
        if (cancelled) {
          if (ownsStream) {
        ...
        if (ownsStream) onStreamReady?.(stream)
