...
/**
 * Read one file from the agent's own folder, from the computer the agent
 * lives on (P3.5). Same shape as `/api/sessions/:id/file` (`?path=`,
 * optional `?base=1` for the diff "old" side). Read-only: editing from the
 * agent view is out of scope (docs/agents-view-spec.md §7), and a git
 * agent's checkout is changed through executions.
 */
