...
import { ensureHarnessSettings, upsertHarnessSettings } from '@/lib/db/queries';
...
async function handleGET(request: Request) {
  ...
  const discovery = await getHarnessModels(provider, { cwd, refresh });
  ...
  let settings = ensureHarnessSettings(provider);
  if (refresh) {
    settings = upsertHarnessSettings({
      ...settings,
