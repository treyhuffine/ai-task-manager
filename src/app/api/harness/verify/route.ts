...
export interface HarnessVerifyResponse {
  ok: boolean;
  status: string;
  durationMs: number;
  costUsd: number | null;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const catalog = await getHarnessModelCatalog(harness, { cwd, refresh: true });
    ...
    // next /api/harness/auth call re-reads from source (e.g., user just
    // finished `claude login` and the verify confirmed it works).
    if (ok) clearAuthCache();

    const payload: HarnessVerifyResponse = {
      ok,
