/**
 * POST /api/webhooks/triggers/<publicId>
 *
 * External webhook intake for triggers with `kind='webhook'`. The
 * publicId in the path identifies the trigger; HMAC-SHA256 over the
 * raw body authenticates the request (see `lib/triggers/webhook.ts`
 ...
 *   - 404 — unknown publicId or trigger disabled
 *   - 413 — body exceeds 256 KiB
 *
 * The endpoint is exempted from the bearer-token middleware in
 * `src/proxy.ts`. It's the trigger's HMAC signature that gates entry.
 */
...
export async function POST(request: NextRequest, ctx: RouteContext) {
  ...
  const auth = verifyWebhookRequest({
    ...
    storedSecretHash: trigger.webhookSecretHash,
  });
  if (!auth.ok) {
    return Response.json(
  ...
  try {
    ...
    console.error('[POST /api/webhooks/triggers/:public_id]', err);
