/**
 * Webhook intake helpers for triggers with `kind='webhook'`.
 *
 * Identity:  `webhookPublicId` — random 24-byte URL-safe slug; the
 *            path segment of `/api/webhooks/triggers/<publicId>`.
...
export interface NewWebhookCredentials {
  ...
  secret: string;
  /** Stored on the trigger row. sha256(secret) hex. */
  secretHash: string;
}

/** Generate a fresh credential pair for a new webhook trigger. */
