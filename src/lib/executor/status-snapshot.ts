 ...
 *   - `adapter.ts`        `Symbol.for('@ri/executor-state')`      → runtime/background task state
 *   - `pending-input.ts`  `Symbol.for('@ri/pending-input-state')` → `bySession: Map<string, Set<string>>`
 *
...
const EXECUTOR_STATE_KEY = Symbol.for('@ri/executor-state');
// Mirror of pending-input.ts's STATE_KEY (bySession slice only).
const PENDING_STATE_KEY = Symbol.for('@ri/pending-input-state');
