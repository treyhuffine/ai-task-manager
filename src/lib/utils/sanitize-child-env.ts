/**
 * Sanitize the environment for processes Ri spawns on behalf of the
 * user — preview dev servers, agent terminals, anything downstream of
 * Ri's own Node process.
 *
 * Why this matters: Ri runs as a Next.js worker process. Next exports
 ...
 *   - `PORT=4224` makes child dev servers try to bind to Ri's own port.
 *   - `__NEXT_PRIVATE_ORIGIN` / `NEXT_PRIVATE_WORKER` make a child Next
 *     think it's a worker of the parent Ri server, producing subtle
...
const STATIC_DROP = new Set<string>([
  // Ri's own run mode must not cross into the user's app processes. If Ri
  ...
  // Network / binding — would force the child onto Ri's port.
  'PORT',
  'HOST',

  // Next.js internal worker plumbing — leaks Ri's identity into a
  ...
  // Portless inheritance — if Ri itself was started under
  // `portless run`, these point at Ri's allocation, not the child's.
  'PORTLESS_URL',
 ...
 */
const DROP_PREFIXES = ['__NEXT_', 'NEXT_PRIVATE_', 'RI_'];
