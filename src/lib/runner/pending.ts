 ...
 * UI answers. The caller (the local runner) hands the resolved response back to
...
function notify(sessionId: string): void {
  // A publish for screens only: with no sink there is no one to tell.
  if (hasRunnerSink()) runnerSink().signal(sessionId, { type: 'pending_changed', pending: listForSession(sessionId) });
}
