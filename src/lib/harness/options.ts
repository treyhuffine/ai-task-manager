...
export function effortOptionsForModel(
  ...
  if (!isKnownHarnessId(harness)) return [];
...
/**
 * Narrow an untyped value (a request body, a query param) to a harness id,
 * or throw. Accepts every harness this build knows, including ones a rollout
 * flag switched off, so stored history always resolves. Values already typed
 * `HarnessId` (every chat, trigger and run row) don't need this.
 */
export function requireHarnessId(harness: string | null | undefined): ProviderId {
  if (!harness) throw new Error('A harness is required');
  if (!isKnownHarnessId(harness)) throw new Error(`Unknown harness: ${harness}`);
  return harness;
}
 ...
 * (see `getHarnessModelCatalog`), so they are catalog members here, not a
 * separate exemption.
 */
...
export function explicitEffortForModel(
  ...
  const providerId: HarnessId | null = isKnownHarnessId(harness) ? harness : null;
  ...
  if (supported.has(DEFAULT_EFFORT)) return DEFAULT_EFFORT;
  return options[0]?.id ?? DEFAULT_EFFORT;
}

export interface ExplicitHarnessSelection {
  providerId: ProviderId;
  model: string;
  variant: string | null;
...
export function explicitHarnessSelection(
  providerId: ProviderId,
  preferred: {
    model?: string | null;
    variant?: string | null;
  ...
  return {
    ...
    effort: harnessSupportsEffort(providerId)
      ? explicitEffortForModel(providerId, model, preferred.effort)
      : null,
  };
}
...
export function findModelOption(harness: string, modelId: string | null): ModelOption | null {
  ...
  if (!isKnownHarnessId(harness)) return null;
  const list = MODEL_OPTIONS[harness];
  return list.find((m) => m.id === modelId) ?? null;
}

export function harnessSupportsEffort(harness: string | null | undefined): boolean {
  return isKnownHarnessId(harness) && HARNESS_REGISTRY[harness].maximumCapabilities.reasoningEffort;
}

// ─── Providers ────────────────────────────────────────────────
// The selectable harnesses. One vocabulary everywhere: the `HarnessId` values
// stored on chats, triggers, runs and `user_state.default_harness`, which are
// also the agentex provider ids and the MODEL_OPTIONS keys. Single source of
// truth for the onboarding step, the settings selector, and the connection
// check.

/** A harness id under the name the provider-facing APIs use. */
 ...
 */
export const PRE_RECONCILE_BUNDLED_IDS: Record<HarnessId, string[]> = {
...
export function reconcileEnabledModels(
  ...
  const seenBefore = known ?? PRE_RECONCILE_BUNDLED_IDS[providerId] ?? [];
