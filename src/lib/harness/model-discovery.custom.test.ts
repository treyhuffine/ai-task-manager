 ...
 * allowlist route, the dispatch preflight — asks `getHarnessModelCatalog`
...
let discovery: typeof import('@/lib/harness/model-discovery');
...
beforeAll(async () => {
  ...
  discovery = await import('@/lib/harness/model-discovery');
...
describe('customModelCatalog', () => {
  ...
  it('is empty for a provider with no settings row, without creating one', () => {
    ...
    expect(q.getHarnessSettings('codex')).toBeUndefined();
  });
