...
describe('agent effort options', () => {
  ...
  it('keeps Claude on its provider-supported levels', () => {
    expect(effortOptionsForModel('claude', null).map((option) => option.id)).toEqual([
      'low',
      'medium',
      'high',
      'xhigh',
  ...
  it('names the top rung the way each provider names it', () => {
    const claude = effortOptionsForModel('claude', null).at(-1);
  ...
  it('resolves null values to an explicit model and effort', () => {
    expect(explicitHarnessSelection('claude')).toEqual({
      providerId: 'claude',
      model: 'opus',
      variant: null,
      effort: 'medium',
  ...
  it('keeps model variants independent from reasoning effort', () => {
    ...
    expect(explicitHarnessSelection('opencode', {
...
describe('pinned model ids', () => {
  ...
  it('resolves a pin like any other catalog model once merged in', () => {
    const catalog = [...MODEL_OPTIONS.claude, customModelOption('claude-opus-4-8')];
    expect(explicitHarnessSelection('claude', { model: 'claude-opus-4-8' }, catalog)).toEqual({
      providerId: 'claude',
