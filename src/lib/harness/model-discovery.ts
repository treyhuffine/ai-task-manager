...
export async function getHarnessModels(
  ...
  try {
    ...
    const value: HarnessModelsResponse = {
 ...
 * keeps the discovered metadata (see the merge order in `getHarnessModelCatalog`).
 */
export function customModelCatalog(providerId: ProviderId): ModelOption[] {
  return (getHarnessSettings(providerId)?.customModels ?? []).map(customModelOption);
}
...
export function clearHarnessModelCache(providerId?: ProviderId): void {
  if (!providerId) {
    cache.clear();
    return;
  }
