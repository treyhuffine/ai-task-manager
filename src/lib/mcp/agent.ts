...
Interpret what the message means and decide what to do. Use read tools first when you need to find an ID or disambiguate. New tasks land in Todo by default. Task status is consider, todo, in_progress, done, or archived, and "current" work is the derived union of todo plus in_progress. Use completeTask (not updateTask) for marking things done, so a recurring task logs one completion and advances to its next occurrence. updateTask changes content and metadata only and cannot set status. Lifecycle moves like start, archive, or reopen go through the transition path (transition_task), and runtime events never change a task's lifecycle on their own. Prefer archiving over deleting unless the caller explicitly asks to delete.
